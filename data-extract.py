...
def process_file(args):
    ...
    with lzma.open(file_path, "rt", encoding="utf-8") as infile:  # lzma to open compressed file, read text
        text = infile.read()
...
def process_files_in_parallel(files, folder_path, output_file):
    vocab = set()
    with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:   # 4 CPUs work in parallel
        ...
        for characters in tqdm(executor.map(process_file, args), total=len(files)):   
            # extract characters set for each arg using process_file func (parallel) (tqdm progress bar)
